...
from blastmini.scoring import (HitScorer, ScoredHit, format_hits_as_text,
                               format_hits_as_tsv)
...
def test_hit_scorer_score_hits():
    ...
    scored = scorer.score_hits(
        hits, query_length=200, db_total_length=500, progress=False)
    assert len(scored) == 2
...
def test_format_hits_as_text():
    ...
    scored = ScoredHit(hit, raw_score=100,
                       identity_percent=95.0, coverage_percent=50.0)
